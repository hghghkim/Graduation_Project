...
%PCA1 Perform the PCA algorithm % 
...
if ~exist('no_dims', 'var') no_dims = 2; 
end
...
if size(X, 2) < size(X, 1)
    ...
    if ~(size(X, 2) < size(X, 1)) 
        M = (X' * M) .* repmat((1 ./ sqrt(size(X, 1) .* lambda))', [size(X, 2) 1]); 
        % normalize in order to get eigenvectors of covariance matrix
    end
    
    mappedX = X * M; % Store information for out-of-sample extension 
    mapping.M = M; 
